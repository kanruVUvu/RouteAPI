...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en">
    <head>
        <title>3ev Vehicle Tracking Maps: Best Route Tracker</title>
        ...
        <style type="text/css">
            ...
            a, img { outline:none; border:none; color:#047CC8; text-decoration:none}
            ...
            .textbox{width: 229px; margin:0 10px 5px 0px;padding:5px;border:1px solid #ddd;color:#555}
            select{color:#555;border:1px solid #ddd;}
...
/*route icons*/
.leaflet-routing-icon {background-image: url('route_advices.svg');
 -webkit-background-size: 455px 20px;background-size: 455px 20px;background-repeat: no-repeat;margin: 0;content: '';display: inline-block;vertical-align: top; width: 20px;height: 20px;}
.leaflet-routing-alt-minimized .leaflet-routing-icon { background-image: url('https://apis.mapmyindia.com/map_v3/osrm.directions.icons.color.svg');}
.leaflet-routing-icon.lanes.invalid {filter: invert(50%);}.leaflet-routing-icon-continue{ background-position: 2px 0px; }.leaflet-routing-icon-sharp-right{ background-position: -24px 0px; }
.leaflet-routing-icon-turn-right{ background-position: -50px 0px; }.leaflet-routing-icon-bear-right{ background-position: -74px 0px; }.leaflet-routing-icon-u-turn{ background-position: -101px 0px; }.leaflet-routing-icon-end{ background-position: -429px 0px; }
.leaflet-routing-icon-sharp-left{ background-position: -127px 0px; }.leaflet-routing-icon-turn-left{ background-position: -150px 0px; }.leaflet-routing-icon-bear-left{ background-position: -175px 0px; }.leaflet-routing-icon-depart{ background-position: -202px 0px; }
.leaflet-routing-icon-enter-roundabout { background-position: -227px 0px; }.leaflet-routing-icon-arrive{ background-position: -253px 0px; }.leaflet-routing-icon-via{ background-position: -278px 0px; }.leaflet-routing-icon-fork{ background-position: -305px 0px; }
.leaflet-routing-icon-ramp-right{ background-position: -331px 0px; }.leaflet-routing-icon-ramp-left{ background-position: -352px 0px; }.leaflet-routing-icon-merge-left{ background-position: -376px 0px; }.leaflet-routing-icon-merge-right{ background-position: -403px 0px; }
...
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $viaa1 = $_POST['viaaddress1'];
}
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://eu1.locationiq.com/v1/search.php?key=d838f0c567a016&q=$viaa1&format=json",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
curl_close($curl);
$sarray1= json_decode($response, true, $depth = 512, $options = 0 );
$addresslength1 = strlen($viaa1);
$viaaddresslat1=$sarray1[0]['lat'];
$viaaddresslon1=$sarray1[0]['lon'];
?>
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $viaa2 = $_POST['viaaddress2'];
}
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://eu1.locationiq.com/v1/search.php?key=d838f0c567a016&q=$viaa2&format=json",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
curl_close($curl);
$sarray2= json_decode($response, true, $depth = 512, $options = 0 );
//print_r($sarray2[0]['lat']);
$addresslength2 = strlen($viaa2);
$viaaddresslat2=$sarray2[0]['lat'];
$viaaddresslon2=$sarray2[0]['lon'];
?>
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $viaa3 = $_POST['viaaddress3'];
}
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://eu1.locationiq.com/v1/search.php?key=d838f0c567a016&q=$viaa3&format=json",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
curl_close($curl);
$sarray3= json_decode($response, true, $depth = 512, $options = 0 );
//print_r($sarray3);
//print_r($sarray3[0]['lat']);
$addresslength3 = strlen($viaa3);
$viaaddresslat3=$sarray3[0]['lat'];
$viaaddresslon3=$sarray3[0]['lon'];
?>
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $viaa4 = $_POST['viaaddress4'];
}
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://eu1.locationiq.com/v1/search.php?key=d838f0c567a016&q=$viaa4&format=json",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
curl_close($curl);
$sarray4= json_decode($response, true, $depth = 512, $options = 0 );
//print_r($sarray4);
//print_r($sarray4[0]['lat']);
$addresslength4 = strlen($viaa4);
$viaaddresslat4=$sarray4[0]['lat'];
$viaaddresslon4=$sarray4[0]['lon'];
?>
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $viaa5 = $_POST['viaaddress5'];
}
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://eu1.locationiq.com/v1/search.php?key=d838f0c567a016&q=$viaa5&format=json",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
curl_close($curl);
$sarray5= json_decode($response, true, $depth = 512, $options = 0 );
//print_r($sarray5);
//print_r($sarray1[0]['lat']);
$addresslength5 = strlen($viaa5);
$viaaddresslat5=$sarray5[0]['lat'];
$viaaddresslon5=$sarray5[0]['lon'];
?>
<div style="border-bottom: 1px solid #e9e9e9;padding:10px 12px;background:#fff;"><span style="font-size: 20px">3ev Vehicle Tracking Maps:</span> <span style="font-size:16px;color:#777">Best Route Tracker</span></div>
        <div id="menu">
            <div style="padding: 0 12px 0 17px;font-size:11px;">
                <div>
                    <div style="padding:10px 0 0px 0;font-size:13px;width:250px;">API:<select id="api_name" style="margin-left:24px;width:75%;padding:4px" onchange="get_route_result()"><option value="route_adv">route_adv</option><option value="route_eta">route_eta</option><option value="route_traffic">route_traffic</option></select></div>
                    <div style="padding:10px 0 10px 0;font-size:13px;width:250px;">Profile:<select id="Profile" style="margin-left:5px;width:75%;padding:4px" onchange="get_route_result()"><option value="driving">driving</option><option value="biking"  >biking</option><option value="trucking"   >trucking</option><option value="walking"  >walking</option></select></div>
                </div>
               <form name='submit-to-google-sheet' id='tresbo' method='post' action="<?php echo $_SERVER['PHP_SELF'];?>">
                <div style="padding: 5px 0;font-size:13px;color:#222;border-top: 2px solid #ddd;">Vehicle Number</div>
                <input type="text" class="textbox" id="vno" name="vehicleno" value='<?php if(isset($_POST['vehicleno'])) echo $_POST['vehicleno']; ?>' placeholder="e.g:KA01AH4356" autocomplete="off" /><br/>
               <div style="padding: 5px 0;font-size:13px;color:#222;border-top: 2px solid #ddd;">Start points</div> 
                <input type="text" class="textbox" id="start" value='<?php echo $value1 ?>,<?php echo $value2 ?>' placeholder="e.g:28.612960,77.229455" autocomplete="off" />
                 <!--<div style="padding:0px 0 5px 0;font-size:13px;">Via points<br>(Points seprated by semi colon) </div> --> 
                <input type="hidden" class="textbox"  id="via" name="viap" value='<?php  echo $viaaddresslat1 ?>,<?php echo $viaaddresslon1 ?>;<?php echo $viaaddresslat2 ?>,<?php echo $viaaddresslon2 ?>;<?php echo $viaaddresslat3 ?>,<?php echo $viaaddresslon3 ?>;<?php echo $viaaddresslat4 ?>,<?php echo  $viaaddresslon4 ?>;<?php echo $viaaddresslat5 ?>,<?php echo $viaaddresslon5 ?>' placeholder="e.g:28.570841,77.325929" autocomplete="off" />
                    <!-- <div style="padding:5px 0 5px 0;font-size:13px;color:#222">Destination points</div> -->
                    <div><input type="hidden" class="textbox"  id="destination" name="desp" value='<?php if(isset($_POST['desp'])) echo $_POST['desp']; ?>'  placeholder="e.g:27.157015,77.991600" autocomplete="off" /></div><br>
                     <div style="padding:0px 0 5px 0;font-size:13px;">Via Address1<br> </div>
                <input type="text" class="textbox"  id="viaadd1" name="viaaddress1" value='<?php if(isset($_POST['viaaddress1'])) echo $_POST['viaaddress1']; ?>' placeholder="e.g:HSR Layout " autocomplete="off" /><br>
                <div style="padding:0px 0 5px 0;font-size:13px;">Via Address2<br> </div>
                <input type="text" class="textbox"  id="viaadd2" name="viaaddress2" value='<?php if(isset($_POST['viaaddress2'])) echo $_POST['viaaddress2']; ?>' placeholder="e.g:HSR Layout " autocomplete="off" /><br>
                <div style="padding:0px 0 5px 0;font-size:13px;">Via Address3<br> </div>
                <input type="text" class="textbox"  id="viaadd3" name="viaaddress3" value='<?php if(isset($_POST['viaaddress3'])) echo $_POST['viaaddress3']; ?>' placeholder="e.g:HSR Layout " autocomplete="off" /><br>
                <div style="padding:0px 0 5px 0;font-size:13px;">Via Address4<br> </div>
                <input type="text" class="textbox"  id="viaadd4" name="viaaddress4" value='<?php if(isset($_POST['viaaddress4'])) echo $_POST['viaaddress4']; ?>' placeholder="e.g:HSR Layout " autocomplete="off" /><br>
                <div style="padding:0px 0 5px 0;font-size:13px;">Via Address5<br> </div>
                <input type="text" class="textbox"  id="viaadd5" name="viaaddress5" value='<?php if(isset($_POST['viaaddress5'])) echo $_POST['viaaddress5']; ?>' placeholder="e.g:HSR Layout " autocomplete="off" /><br>
                    <div style="padding:5px 0 5px 0;font-size:13px;color:#222">Destination Address</div>
                    <div><input type="text" class="textbox"  id="destaddr" name="desaddress" value='<?php if(isset($_POST['desaddress'])) echo $_POST['desaddress']; ?>' placeholder="e.g:HSR Layout" autocomplete="off" /></div>
                   <div style="padding:5px 0 5px 0;font-size:13px;color:#222">Input Address Format:<label class="radio-inline"><input type="radio" name="delimiter" id="delimiter-csv" checked>Area Name</label></div>
                     <div style="margin:20px 0 5px 0px;"><input type="button" value="Update" id="submit" class="btn btn-lg btn-primary"></div><br> 
                    <div style="padding:10px 0 15px 0;font-size:13px;width:250px;"><div style="float:left">Advices</div><div style="float:right"><select id="advices_o" style="width:165px" onchange="get_route_result()"><option value="true" selected="">With advices</option><option value="false">Without advices</option></select></div></div>
                    <div style="padding:10px 0 15px 0;font-size:13px;width:250px;"><div style="float:left">Avoids</div><div style="float:right"><select id="avoids" style="width:165px" onchange="get_route_result()"><option value="" selected="">No Avoid</option><option value="toll" >Toll roads</option><option value="motorway">Highways</option><option value="ferry">Ferries</option></select></div></div>                    
                    <div style="padding:10px 0 15px 0;font-size:13px;width:250px;"><div style="float:left">With alternatives route</div><div style="float:right"><select id="alternatives_o" style="width:96px" onchange="get_route_result()"><option value="true" selected="">True</option><option value="false">False</option></select></div></div>
                    <div style="padding:10px 0 15px 0;font-size:13px;width:250px;" id="rtype_dv"><div style="float:left">rtype</div><div style="float:right"><select id="rtype" style="width:96px" onchange="get_route_result()"><option value="1" >1 (Shortest)</option><option value="0">0 (Optimal Route)</option></select></div></div>
                    
                    <div style="padding:10px 0 15px 0;font-size:13px;width:250px;" id="region_dv"><div style="float:left">Region</div><div style="float:right"><select id="region" style="width:96px" onchange="get_route_result()"><option value="ind" selected="">India</option><option value="bgd">Bangladesh</option><option value="btn">Bhutan</option><option value="npl">Nepal</option><option value="lka">Sri Lanka</option></select></div></div>
                    
                    <div style="margin:20px 0 5px 0px;"><button onclick="get_route_result()" >Get Route</button></div></form><br>
                        <div id="alternate" style="padding:2px 5px 2px 5px;border:1px solid #ccc;border-radius: 10px;width:254px;display: none"><label><input type="checkbox" id="alternatives"  onclick="alternative_route()" style="float: left"> <div style="padding:3px 0px 3px 10px;float:left">Show available alternative route</div></label></div>
                        </div>
  ...
  <script>
      	...
      	var token = '0edd1d5ed308b289857c';	
	...
	//	if( $('#output-addr').prop('checked') ){
		//	str = addr+' '+str;
	//	}
		var n=el.val(el.val().replace(addr+delimiter, str));	
	}
	...
	$(document).ready(function(){
			...
			console.log(data);
			$('#destination').val(rawData+delimiter);	
        ...
        <!--put your map api javascript url with key here-->
         <script src="https://apis.mapmyindia.com/advancedmaps/v1/i19tolx5gtklubpb1tqsi9i9m4up53ay/map_load?v=1.3"></script>
        <script>
        ...
        var route_api_url='https://apis.mapmyindia.com/advancedmaps/v1/5ts1zn15hjj4t2qs4q5gfp7erdamrpw9/';
        window.onload = function(){
            
            try{map=new MapmyIndia.Map('map-container',{zoomControl: true,hybrid:true,traffic:true });}catch(e){alert('Please check map key');}
        ...
        function getUrlResult(api_url) {
           if(leaflet_polyline) map.removeLayer(leaflet_polyline);
                    ...
                    $.ajax({
                        type: "POST",
                        dataType: 'text',
                        url: "https://www.mapmyindia.com/api/advanced-maps/doc/sample/getResponse",
                        async: false,
                        data: {
